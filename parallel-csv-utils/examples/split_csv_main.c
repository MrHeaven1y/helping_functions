...
void print_timings(double total_elapsed, long fsize) {
    ...
    if (total_elapsed < 0.001) {
        printf("Operation completed too quickly for precise timing\n");
        return;
    }
    ...
    if (fsize > 0) {
        ...
        printf("Throughput: Could not determine file size\n");
    }
...
int main(int argc, char *argv[]) {
    ...
    char ext[32] = ".csv";
    char delim[32] = ",";
    unsigned chunk_size = 30000;  // Optimized default chunk size
    int num_threads;
    int max_threads = omp_get_max_threads();
    int operation_mode = 0;  // 0=process, 1=copy, 2=parallel copy
    ...
    if(argc > 1) {
        ...
        if(argc > 7) operation_mode = atoi(argv[7]);
        ...
        printf("CSV File Processor Configuration\n");
        printf("------------------------------\n");
        ...
        get_string_input("File extension [default '.csv']: ", ext, sizeof(ext));
        get_string_input("Delimiter [default ',']: ", delim, sizeof(delim));
        
        printf("\nOperation Modes:\n");
        printf("0 - Process CSV (parse and transform data)\n");
        printf("1 - Fast Copy (single-threaded file splitting)\n");
        printf("2 - Parallel Copy (multi-threaded file splitting)\n");
        operation_mode = get_positive_integer("Select operation mode (0-2): ");
        
        // Set optimized chunk sizes based on mode
        if(operation_mode == 0) {
            printf("Recommended chunk size: 20,000 lines\n");
            chunk_size = get_positive_integer("Chunk size (lines per file): ");
        } else if(operation_mode == 1) {
            printf("Recommended chunk size: 50,000 lines\n");
            chunk_size = get_positive_integer("Chunk size (lines per file): ");
        } else {
            printf("Recommended chunk size: 200,000 lines\n");
            chunk_size = get_positive_integer("Chunk size (lines per file): ");
        }
    ...
    if(chunk_size <= 0) {
        chunk_size = (operation_mode == 2) ? 200000 : 
                    (operation_mode == 1) ? 50000 : 20000;
    }
    ...
    printf("Operation mode: %s\n", 
          operation_mode == 0 ? "Process CSV" :
          operation_mode == 1 ? "Fast Copy" : "Parallel Copy");
    printf("Max available threads: %d\n", max_threads);
    
    int result;
    switch(operation_mode) {
        case 0:  // Full CSV processing
            result = parallel_process_csv(fname, outputfname, ext, delim, chunk_size, num_threads);
            break;
        case 1:  // Single-threaded copy
            result = copyData(fname, outputfname, ext, delim, chunk_size);
            break;
        case 2:  // Multi-threaded copy
            result = parallel_copy_data(fname, outputfname, ext, delim, chunk_size, num_threads);
            break;
        default:
